...
#args <- commandArgs()
# 1. path to file
# 2. number of loci (be aware that they should be at the end of table followed by
#                     'number of samples')
# 3. number of collumn with distance
# 4. number of collumn with unique values for different hybrid zones

## decide about type of the model here
plotting <- T
scaling_args <- c('none')
tails_args <- c('none')
models_args <- list()
...
### Contemporary ###
#setwd("~/Dropbox/MHC_projekt/Shared_MHC_project/Triturus/hzar/Paper_work")
cwd <- getwd()
args <- c("Anguis_hzar_input.csv",5,3,1)
markers <- c("HI","HII","QI","Q2","genomic")
### ### ### ### ###
